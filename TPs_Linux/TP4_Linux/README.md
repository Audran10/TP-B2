...
```
[audran@docker1 ~]$ sudo dnf install dnf-plugins-core -y
Complete!
[audran@docker1 ~]$ sudo dnf config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo
Adding repo from: https://download.docker.com/linux/centos/docker-ce.repo
[audran@docker1 ~]$ sudo dnf install docker-ce docker-ce-cli containerd.io docker-compose-plugin
Complete!
```
- démarrer le service `docker` avec une commande `systemctl`
```
[audran@docker1 ~]$ sudo systemctl start docker
```
- ajouter votre utilisateur au groupe `docker`
  ...
  ```
  [audran@docker1 ~]$ sudo usermod -aG docker audran
  ```
...
- lancer un conteneur `nginx`
  ...
  ```
  [audran@docker1 ~]$ sudo vim toto.conf
  server {
    listen 8888;
  ...
  ```
  [audran@docker1 ~]$ sudo nano index.html
  <!DOCTYPE html>
  <html>
  <head>
...
## Commande complète :
```
[audran@docker1 ~]$ docker run -p 9000:8888 -v /home/audran/toto.conf:/etc/nginx/conf.d/toto.conf -v /home/audran/index.html:/var/www/toto/index.html --cpus="0.5" --memory="7MB" --name web nginx
```
...
## Création dossier :
```
[audran@docker1 ~]$ mkdir image/
```

## Création fichier de conf et html :
```
[audran@docker1 image]$ sudo nano apacheConf.conf
...
```
[audran@docker1 image]$ sudo nano index.html
<!DOCTYPE html>
<html>
<head>
...
## Création Dockerfile :
```
[audran@docker1 image]$ sudo nano Dockerfile
